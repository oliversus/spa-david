...
contains
  ...
  subroutine initialise_veg( tsteps , A )
    ...
    use veg,                only: ess, flux, gppt, lafrac, lai, layerht, LMA, &
                                  LWPstore, respt, soiletmm, transt, species_qi, initial_Cf, initial_Cr, initial_Cw
    use enkf
    use math_tools
    ...
    do memb = 1 , nrens
       if (species_qi) A( 8  , memb) = initial_Cf
       if (species_qi) A( 9  , memb) = initial_Cw
       if (species_qi) A( 10 , memb) = initial_Cr
